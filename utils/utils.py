...
class NeighborSampler:
    ...
    def find_co_neighbors_before(self, node_src_id: int, node_dst_id: int, interact_time: float, return_sampled_probabilities: bool = False, max_input_sequence_length: int = 0):
        """
        ...
        src_i = np.searchsorted(self.nodes_neighbor_times[node_src_id], interact_time)
        dst_i = np.searchsorted(self.nodes_neighbor_times[node_dst_id], interact_time)

        if return_sampled_probabilities:
            return self.nodes_neighbor_ids[node_src_id][:src_i], self.nodes_edge_ids[node_src_id][:src_i], self.nodes_neighbor_times[node_src_id][:src_i], self.nodes_neighbor_sampled_probabilities[node_src_id][:src_i], \
                   self.nodes_neighbor_ids[node_dst_id][:dst_i], self.nodes_edge_ids[node_dst_id][:dst_i], self.nodes_neighbor_times[node_dst_id][:dst_i], self.nodes_neighbor_sampled_probabilities[node_dst_id][:dst_i]
        else:
            if len(self.nodes_neighbor_times[node_src_id][:src_i]) == 0 or len(self.nodes_neighbor_times[node_dst_id][:dst_i]) == 0:
                return self.nodes_neighbor_ids[node_src_id][:src_i], self.nodes_edge_ids[node_src_id][:src_i], self.nodes_neighbor_times[node_src_id][:src_i], None, \
                       self.nodes_neighbor_ids[node_dst_id][:dst_i], self.nodes_edge_ids[node_dst_id][:dst_i], self.nodes_neighbor_times[node_dst_id][:dst_i], None
            else:
                if max_input_sequence_length == 32:
                    src_index_interact_nodes = np.where(self.nodes_neighbor_ids[node_src_id][:src_i] == node_dst_id)[0][-32:]
                    dst_index_interact_nodes = np.where(self.nodes_neighbor_ids[node_dst_id][:dst_i] == node_src_id)[0][-32:]
                if max_input_sequence_length == 64:
                    src_index_interact_nodes = np.where(self.nodes_neighbor_ids[node_src_id][:src_i] == node_dst_id)[0][-64:]
                    dst_index_interact_nodes = np.where(self.nodes_neighbor_ids[node_dst_id][:dst_i] == node_src_id)[0][-64:]
                elif max_input_sequence_length == 128:
                    src_index_interact_nodes = np.where(self.nodes_neighbor_ids[node_src_id][:src_i] == node_dst_id)[0][-128:]
                    dst_index_interact_nodes = np.where(self.nodes_neighbor_ids[node_dst_id][:dst_i] == node_src_id)[0][-128:]
                if len(src_index_interact_nodes) != 0:
                    src_select_ids = src_index_interact_nodes
                    return_nodes_src_neighbor_ids, return_nodes_src_edge_ids, return_nodes_src_neighbor_time = \
                        self.nodes_neighbor_ids[node_src_id][:src_i][src_select_ids], self.nodes_edge_ids[node_src_id][:src_i][src_select_ids], self.nodes_neighbor_times[node_src_id][:src_i][src_select_ids]
                else:
                    return_nodes_src_neighbor_ids, return_nodes_src_edge_ids, return_nodes_src_neighbor_time = \
                        self.nodes_neighbor_ids[node_src_id][:src_i], self.nodes_edge_ids[node_src_id][:src_i], self.nodes_neighbor_times[node_src_id][:src_i]
                if len(dst_index_interact_nodes) != 0:
                    dst_select_ids = dst_index_interact_nodes
                    return_nodes_dst_neighbor_ids, return_nodes_dst_edge_ids, return_nodes_dst_neighbor_time = \
                        self.nodes_neighbor_ids[node_dst_id][:dst_i][dst_select_ids], self.nodes_edge_ids[node_dst_id][:dst_i][dst_select_ids], self.nodes_neighbor_times[node_dst_id][:dst_i][dst_select_ids]
                else:
                    return_nodes_dst_neighbor_ids, return_nodes_dst_edge_ids, return_nodes_dst_neighbor_time = \
                        self.nodes_neighbor_ids[node_dst_id][:dst_i], self.nodes_edge_ids[node_dst_id][:dst_i], self.nodes_neighbor_times[node_dst_id][:dst_i]
                return return_nodes_src_neighbor_ids, return_nodes_src_edge_ids, return_nodes_src_neighbor_time, None, \
                       return_nodes_dst_neighbor_ids, return_nodes_dst_edge_ids, return_nodes_dst_neighbor_time, None

    def get_co_historical_neighbors(self, node_src_ids: np.ndarray, node_dst_ids: np.ndarray, node_interact_times: np.ndarray, max_input_sequence_length: int):
        """
        ...
        :return:
        """
        nodes_src_neighbor_ids, nodes_src_edge_ids, nodes_src_neighbor_times = [], [], []
        nodes_dst_neighbor_ids, nodes_dst_edge_ids, nodes_dst_neighbor_times = [], [], []
        ...
        for idx, (node_src_id, node_dst_id, node_interact_time) in enumerate(zip(node_src_ids, node_dst_ids, node_interact_times)):
            # find neighbors that interacted with node_id before time node_interact_time
            node_src_neighbor_ids, node_src_edge_ids, node_src_neighbor_times, node_src_neighbor_sampled_probabilities, \
            node_dst_neighbor_ids, node_dst_edge_ids, node_dst_neighbor_times, node_dst_neighbor_sampled_probabilities = \
                self.find_co_neighbors_before(node_src_id=node_src_id, node_dst_id=node_dst_id, interact_time=node_interact_time,
                                              return_sampled_probabilities=self.sample_neighbor_strategy == 'time_interval_aware', max_input_sequence_length=max_input_sequence_length)
            nodes_src_neighbor_ids.append(node_src_neighbor_ids)
            nodes_src_edge_ids.append(node_src_edge_ids)
            nodes_src_neighbor_times.append(node_src_neighbor_times)
            nodes_dst_neighbor_ids.append(node_dst_neighbor_ids)
            nodes_dst_edge_ids.append(node_dst_edge_ids)
            nodes_dst_neighbor_times.append(node_dst_neighbor_times)

        # three ndarrays, with shape (batch_size, num_neighbors)
        return nodes_src_neighbor_ids, nodes_src_edge_ids, nodes_src_neighbor_times, \
               nodes_dst_neighbor_ids, nodes_dst_edge_ids, nodes_dst_neighbor_times
...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None, seed: int = None):
        """

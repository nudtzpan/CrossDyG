...
class CrossDyG(nn.Module):

    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, channel_embedding_dim: int, num_layers: int = 2, num_heads: int = 2,
                 dropout: float = 0.1, max_input_sequence_length: int = 512, gamma: float = 0.0, device: str = 'cpu'):
        """
        CrossDyG model.
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        dst_nodes_neighbor_ids_list, dst_nodes_edge_ids_list, dst_nodes_neighbor_times_list = \
            self.neighbor_sampler.get_co_historical_neighbors(node_src_ids=src_node_ids, node_dst_ids=dst_node_ids,
                                                              node_interact_times=node_interact_times, max_input_sequence_length=self.max_input_sequence_length)
        ...
        # Tensor, shape (batch_size, dst_max_seq_length, channel_embedding_dim)
        ...
        src_max_seq_length = src_padded_nodes_neighbor_node_raw_features.shape[1]
        dst_max_seq_length = dst_padded_nodes_neighbor_node_raw_features.shape[1]

        # Tensor, shape (batch_size, src_max_seq_length + dst_max_seq_length, channel_embedding_dim)
        padded_nodes_neighbor_node_raw_features = torch.cat([src_padded_nodes_neighbor_node_raw_features, dst_padded_nodes_neighbor_node_raw_features], dim=1)
        padded_nodes_edge_raw_features = torch.cat([src_padded_nodes_edge_raw_features, dst_padded_nodes_edge_raw_features], dim=1)
        padded_nodes_neighbor_time_features = torch.cat([src_padded_nodes_neighbor_time_features, dst_padded_nodes_neighbor_time_features], dim=1)
        padded_nodes_neighbor_co_occurrence_features = torch.cat([src_padded_nodes_neighbor_co_occurrence_features, dst_padded_nodes_neighbor_co_occurrence_features], dim=1)

        padded_data = [padded_nodes_neighbor_node_raw_features, padded_nodes_edge_raw_features,
                       padded_nodes_neighbor_time_features, padded_nodes_neighbor_co_occurrence_features]
        # Tensor, shape (batch_size, src_max_seq_length + dst_max_seq_length, num_channels, channel_embedding_dim)
        padded_data = torch.stack(padded_data, dim=2)
        # Tensor, shape (batch_size, src_max_seq_length + dst_max_seq_length, num_channels * channel_embedding_dim)
        padded_data = padded_data.reshape(batch_size, src_max_seq_length + dst_max_seq_length, self.num_channels * self.channel_embedding_dim)

        # Tensor, shape (batch_size, src_max_seq_length + dst_max_seq_length, num_channels * channel_embedding_dim)
        for transformer in self.transformers:
            padded_data = transformer(padded_data)

        # src_padded_data, Tensor, shape (batch_size, src_max_seq_length, num_channels * channel_embedding_dim)
        src_padded_data = padded_data[:, : src_max_seq_length, :]
        # dst_padded_data, Tensor, shape (batch_size, dst_max_seq_length, num_channels * channel_embedding_dim)
        dst_padded_data = padded_data[:, src_max_seq_length: src_max_seq_length + dst_max_seq_length, :]
        # src_padded_data, Tensor, shape (batch_size, num_channels * channel_embedding_dim)
        src_padded_data = torch.mean(src_padded_data, dim=1)
        # dst_padded_data, Tensor, shape (batch_size, num_channels * channel_embedding_dim)
        dst_padded_data = torch.mean(dst_padded_data, dim=1)
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def forward(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray, gamma: float):
        """
        ...
        # Tensor, shape (batch_size, src_max_seq_length, neighbor_co_occurrence_feat_dim, 2, feat_dim)
        src_padded_nodes_neighbor_co_occurrence_features = self.neighbor_co_occurrence_encode_layer((src_padded_nodes_appearances!=0).float().unsqueeze(dim=-1)) * \
                                                                                                    torch.pow(src_padded_nodes_appearances, gamma).unsqueeze(dim=-1)
        src_padded_nodes_neighbor_co_occurrence_features = src_padded_nodes_neighbor_co_occurrence_features.sum(dim=2)
        # Tensor, shape (batch_size, dst_max_seq_length, neighbor_co_occurrence_feat_dim, 2, feat_dim)
        dst_padded_nodes_neighbor_co_occurrence_features = self.neighbor_co_occurrence_encode_layer((dst_padded_nodes_appearances!=0).float().unsqueeze(dim=-1)) * \
                                                                                                    torch.pow(dst_padded_nodes_appearances, gamma).unsqueeze(dim=-1)
        dst_padded_nodes_neighbor_co_occurrence_features = dst_padded_nodes_neighbor_co_occurrence_features.sum(dim=2)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        :param inputs: Tensor, shape (batch_size, max_seq_length, self.attention_dim)
        :return:
        """
        # note that the MultiheadAttention module accept input data with shape (seq_length, batch_size, input_dim), so we need to transpose the input
        # Tensor, shape (max_seq_length, batch_size, self.attention_dim)
        transposed_inputs = inputs.transpose(0, 1)
        # Tensor, shape (batch_size, max_seq_length, self.attention_dim)

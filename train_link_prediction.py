...
if __name__ == "__main__":
    ...
    node_raw_features, edge_raw_features, train_data = get_link_prediction_data(dataset_name=args.train_dataset_name)
    ...
    # get data loaders
    # np.random.seed(0)
    # select_list = np.random.choice(np.arange(len(train_data.src_node_ids)), int(1.0*len(train_data.src_node_ids)), replace=False).tolist()
    # train_idx_data_loader = get_idx_data_loader(indices_list=select_list, batch_size=args.batch_size, shuffle=False)
    ...
    for run in range(args.num_runs):
        ...
        os.makedirs(f"./logs/{args.model_name}/{args.train_dataset_name}/training/{args.save_model_name}/", exist_ok=True)
        # create file handler that logs debug and higher level messages
        fh = logging.FileHandler(f"./logs/{args.model_name}/{args.train_dataset_name}/training/{args.save_model_name}/{str(time.time())}.log")
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone = CrossDyG(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                        time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim,
                                        num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
        ...
        save_model_folder = f"./saved_models/{args.model_name}/{args.train_dataset_name}/{args.save_model_name}/"
        ...
        for epoch in range(args.num_epochs):
            ...
            model.train()
            if args.model_name in ['TGAT', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer', 'CrossDyG']:
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                          dst_node_ids=batch_neg_dst_node_ids,
                                                                          node_interact_times=batch_node_interact_times)
                elif args.model_name in ['CrossDyG']:
                    # get temporal embedding of source and destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                    batch_src_node_embeddings, batch_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
            ...
            save_model_path = os.path.join(save_model_folder, f"{args.model_name}_{epoch}.pkl")
